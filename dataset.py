...
class Dataset(object):
    ...
    def __init__(self, depth, img_size):
        ...
        self.train_data_num = train_data_paths.size
        ...
        self.validation_data_num = validation_data_paths.size/2     # 为什么要除2？ 告诉你是因为label和path下的都TM计算！！！
        # 洗牌
...
#获取valiation数据
    def get_valiation_data(self, batch_size):
        ...
        for i in range(batch_size):
            ...
            assert frames_num-self.depth >= 0, print(frames_num, img_path_list)     # 必须保证至少有depth个帧
            if frames_num-self.depth == 0:      # 帧数与深度相等
                start = 0
            else:
                start = np.random.randint(0, frames_num-self.depth+1)
            for j in range(self.depth):
